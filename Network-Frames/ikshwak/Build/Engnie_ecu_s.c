...
int main()
{
    ...
    printf("Encrypted Random Number: ");
    for(int i = 0; i < 16; i++)
    {
        printf("%02X ", value[i]);
    }
    printf("\n");
    retval = send(client_fd, sendBuffer, sizeof(sendBuffer), 0);
    recv(client_fd, rec_value, sizeof(rec_value), 0);
    printf("Received Encrypted value from Transponder ECU: ");
    for(int i = 0; i < 16; i++)
    {
        printf("%02X ", rec_value[i]);
    }
    printf("\n");
    if(memcmp(value, rec_value, 16) == 0)
    {
        printf("Authentication Successful: Transponder ECU is valid.\n");
    }
    else
    {
        printf("Authentication Failed: Transponder ECU is invalid.\n");
    }
