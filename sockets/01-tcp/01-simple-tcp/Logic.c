...
/* Fixed secret key (shared secret) */
...
static void AddRoundKey(uint8_t *s) {
    for (int i = 0; i < 16; i++)
        s[i] ^= secret_key[i];
}

/* AES encrypt core */
static void AES128_Encrypt(uint8_t *input, uint8_t *output) {
    uint8_t state[16];
